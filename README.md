...
Добро пожаловать в **A2MedAnalyzer** — мощный API, разработанный для анализа геномных и метаболических данных. Этот инструмент использует передовые биоинформатические методы для обработки данных, связанных с генами, выполнения анализа обогащения и визуализации метаболических сетей. A2MedAnalyzer интегрируется с фронтендом, доступным по адресу [https://a2bio.github.io/A2MedAnalyzer](https://a2bio.github.io/A2MedAnalyzer), который предоставляет удобный веб-интерфейс для взаимодействия с API. Ниже приведена подробная информация о доступных эндпоинтах, их использовании и примерах.

## Обзор

A2MedAnalyzer построен на базе Django и интегрируется с внешними API (например, NCBI E-utils) и библиотеками, такими как `pandas`, `gseapy`, `networkx` и `plotly`. Он предоставляет RESTful эндпоинты для обработки загружаемых файлов, аннотаций генов и визуализации сетей. Все эндпоинты доступны через POST-запросы, если не указано иное. Фронтенд проекта, расположенный по адресу [https://a2bio.github.io/A2MedAnalyzer](https://a2bio.github.io/A2MedAnalyzer), позволяет пользователям удобно работать с API через веб-интерфейс, отправлять запросы и получать результаты в визуализированном виде.
