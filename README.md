...
- **Traits**: Позволяет пользователям искать исследования, связанные с определенным признаком заболевания.
- **Filtrate**: Позволяет пользователям загружать TSV-файл для фильтрации и анализа данных генов.
- **Annotation**: Предоставляет форму для загрузки CSV-файла для выполнения анализа обогащения GO и KEGG.
- **NCBI**: Позволяет пользователям вводить символы генов для получения информации о генах из NCBI.
...
- **Ошибки загрузки файла**:
  - Убедитесь, что файл в формате CSV или TSV и не превышает 10 МБ.
- **Таймауты API**:
  - Уменьшите размер входных данных или проверьте интернет-соединение.
